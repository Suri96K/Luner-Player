...
import com.example.pc.lunerplayer.R;


import static android.view.View.*;

public class SaveAndCheackFilesActivity extends AppCompatActivity implements View.OnClickListener {

    SQLiteDatabase db;
    EditText editsearch, editempname, editempmail, editArtist;
    SQLiteDatabase sqlitedb;
    Button Add, Delete, Update, Searchall, Search;
    String name, mailid, Artist, search, artist;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_save_and_cheack_files);

        //Create database,EmployeeDB database name
        db = openOrCreateDatabase("EmployeeDB", Context.MODE_PRIVATE, null);
        //create table Employee
        db.execSQL("CREATE TABLE IF NOT EXISTS Employee(EmpId INTEGER PRIMARY KEY AUTOINCREMENT,EmpName VARCHAR,EmpMail VARCHAR,Artist VARCHAR);");
        ...
        editArtist = (EditText) findViewById(R.id.editText4);
    ...
    public void onClick(View v) {
        ...
        if (v.getId() == R.id.button) {
            ...
            if (editempname.getText().toString().trim().length() == 0 || editempmail.getText().toString().trim().length() == 0 || editArtist.getText().toString().trim().length() == 0) {
                Toast.makeText(this, "Fields cannot be Empty", Toast.LENGTH_SHORT).show();
                return;
            }
            db.execSQL("INSERT INTO Employee(EmpName,EmpMail,Artist)VALUES('" + editempname.getText() + "','" + editempmail.getText() + "','" + editArtist.getText() + "');");
        ...
        else if(v.getId()==R.id.button3)
        {
            ...
            if(c.moveToFirst()) {
                db.execSQL("UPDATE Employee  SET EmpName ='"+ editempname.getText()+"', EmpMail='"+ editempmail.getText()+"',Artist='"+ editArtist.getText()+"' WHERE EmpName ='"+editsearch.getText()+"'");
        ...
        else if (v.getId() == R.id.button2)
        {
            ...
            while(c.moveToNext())
            {
                ...
                buffer.append("Song Name "+c.getString(3)+"\n\n");
            }
        ...
        else if(v.getId()== R.id.button4)
        {
            ...
            if(c.moveToFirst())
            {
                ...
                editArtist.setText(c.getString(3));
            }
            else
            {
                Toast.makeText(this, "Invalid  Name",Toast.LENGTH_SHORT).show();
